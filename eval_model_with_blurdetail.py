...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(os.path.join("results", "blur_detail_trained_with_blurdetail_context.xlsx"), engine='xlsxwriter')

    for seed in SEEDS:
        #weights_file = os.path.join("model_weights", "resnet_110_10_blurdetail_u_" + str(seed) + ".h5")

        checkpoint_filepath = os.path.join("model_weights", "resnet_101_blurdetail_context" + str(seed),
        ...
        for filter in filter_list:
            ...
            _, fil_data, _, fil_label = train_test_split(fil_data, fil_label, test_size=300,
        ...
        res = evaluate_model(checkpoint_filepath, x_test, y_test, bins=15, verbose=True,
                             pickle_file="probs_resnet101_blurdetail_with_blurdetail_" + str(seed), x_val=x_val,
                             y_val=y_val, pickle_path="logits_context", contexts=True)

        res = {"Accuracy": res[0], "ECE": res[1], "MCE": res[2]}
        results = results.append(res, ignore_index=True)

        print(results)
        results.to_excel(writer, sheet_name="blurdetail_cal" + str(seed), index=False)
    writer.save()
