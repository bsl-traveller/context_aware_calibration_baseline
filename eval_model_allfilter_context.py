...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(os.path.join("results", "allfilter_trained_seed_context.xlsx"), engine='xlsxwriter')

    for seed in SEEDS:
        #weights_file = os.path.join("model_weights", "resnet_110_10_blurdetail_u_" + str(seed) + ".h5")

        checkpoint_filepath = os.path.join("model_weights", "resnet_101_allfilter_context_" + str(seed),
                                           "weights-allfilter_context.h5")
        # load data
        ...
        for fil in filter_list:
            ...
            model = ResNet_101(img_shape, NUM_CLASSES, WEIGHT_DECAY)
            ...
            res = evaluate_model(checkpoint_filepath, x_test, y_test, bins=15, verbose=True,
                                 pickle_file="probs_allfilter_context_" + fil + "_" + str(seed), x_val=x_val,
        ...
        print(results)
        results.to_excel(writer, sheet_name="allfilter_" + str(seed), index=False)
    writer.save()
