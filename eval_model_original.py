...
if __name__ == '__main__':
    ...
    writer = pd.ExcelWriter(os.path.join("results", "original_trained_seed.xlsx"), engine='xlsxwriter')

    for seed in SEEDS:
        #weights_file = os.path.join("model_weights", "resnet_110_10_blurdetail_u_" + str(seed) + ".h5")

        checkpoint_filepath = os.path.join("model_weights", "resnet_101_original_" + str(seed),
                                           "weights-original.h5")
        # load data
        ...
        for fil in filter_list:
            ...
            res = evaluate_model(checkpoint_filepath, x_test, y_test, bins=15, verbose=True,
                                 pickle_file="probs_original_" + fil + "_" + str(seed), x_val=x_val,
                                 y_val=y_val, pickle_path="logits")
        ...
        print(results)
        results.to_excel(writer, sheet_name="original_" + str(seed), index=False)
    writer.save()
